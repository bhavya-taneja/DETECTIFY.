...
document.addEventListener('DOMContentLoaded', function() {
    ...
    paragraphInput.addEventListener('input', function() {
        const paragraphs = paragraphInput.value.split('. ');
        const highlightedSentences = paragraphs.map(sentence => {
            const prediction = predictDeception(sentence);
            return prediction === 1 ? `<span class="deceptive">${sentence}</span>` : sentence;
        });
        highlightedParagraph.innerHTML = highlightedSentences.join('. ');
    });

    function predictDeception(sentence) {
        // Implement the logic for making predictions in JavaScript if needed
        // For simplicity, you can leave this as a placeholder
        return 0; // Assuming not deceptive by default
    }
});
